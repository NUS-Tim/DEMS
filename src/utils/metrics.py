...
def iou_score(output, target):
    ...
    sen = get_sensitivity(output_, target_, threshold=0.5)
    pre = get_precision(output_, target_, threshold=0.5)
    spe = get_specificity(output_, target_, threshold=0.5)
    acc = get_accuracy(output_, target_, threshold=0.5)
    fos = 2 * sen * pre / (sen + pre + 1e-8)

    return iou, dsc, sen, pre, fos, spe, acc
